...
GOODS_VOWS = """
  ...
  <div class="card" style="border: 1px solid rgba(255,210,138,.25); background: linear-gradient(180deg, #171121 0%, #1c1429 100%);">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
      ...
      <span style="font-size:12px;color:var(--sub);">京东正品 · 次日达</span>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:14px;">
      <a href="https://search.jd.com/Search?keyword=婚礼誓词本+烫金丝绒+手写对装&enc=utf-8&unionId=2038503768" target="_blank" rel="nofollow sponsored" style="display:block;background:#0e0a14;border:1px solid var(--line);border-radius:10px;padding:14px;text-decoration:none;color:var(--ink);transition:border-color .2s;">
      ...
      <a href="https://search.jd.com/Search?keyword=婚礼丝绒复古双戒盒+高档仪式戒指盒&enc=utf-8&unionId=2038503768" target="_blank" rel="nofollow sponsored" style="display:block;background:#0e0a14;border:1px solid var(--line);border-radius:10px;padding:14px;text-decoration:none;color:var(--ink);transition:border-color .2s;">
        <div style="font-size:24px;margin-bottom:8px;">💍</div>
...
GOODS_CHECKLIST = """
  <!-- 备婚好物推荐模块 (京东联盟) -->
  <div class="card" style="border: 1px solid rgba(255,210,138,.25); background: linear-gradient(180deg, #171121 0%, #1c1429 100%); margin-top:24px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
      <div style="font-size:16px;font-weight:700;color:var(--gold);display:flex;align-items:center;gap:6px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:14px;">
      <a href="https://search.jd.com/Search?keyword=新娘婚礼当天随身应急包38件套&enc=utf-8&unionId=2038503768" target="_blank" rel="nofollow sponsored" style="display:block;background:#0e0a14;border:1px solid var(--line);border-radius:10px;padding:14px;text-decoration:none;color:var(--ink);transition:border-color .2s;">
        <div style="font-size:24px;margin-bottom:8px;">🧰</div>
      ...
      <a href="https://search.jd.com/Search?keyword=接亲堵门游戏道具整蛊大礼包&enc=utf-8&unionId=2038503768" target="_blank" rel="nofollow sponsored" style="display:block;background:#0e0a14;border:1px solid var(--line);border-radius:10px;padding:14px;text-decoration:none;color:var(--ink);transition:border-color .2s;">
        <div style="font-size:24px;margin-bottom:8px;">🎉</div>
...
GOODS_QR = """
  ...
  <div class="card" style="border: 1px solid rgba(255,210,138,.25); background: linear-gradient(180deg, #171121 0%, #1c1429 100%); margin-top:24px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:14px;">
      <a href="https://search.jd.com/Search?keyword=婚礼签到台迎宾水牌定制亚克力相框&enc=utf-8&unionId=2038503768" target="_blank" rel="nofollow sponsored" style="display:block;background:#0e0a14;border:1px solid var(--line);border-radius:10px;padding:14px;text-decoration:none;color:var(--ink);transition:border-color .2s;">
      ...
      <a href="https://search.jd.com/Search?keyword=婚礼高级伴手礼盒定制喜糖包装&enc=utf-8&unionId=2038503768" target="_blank" rel="nofollow sponsored" style="display:block;background:#0e0a14;border:1px solid var(--line);border-radius:10px;padding:14px;text-decoration:none;color:var(--ink);transition:border-color .2s;">
        <div style="font-size:24px;margin-bottom:8px;">🎁</div>
...
def update_pages():
    ...
    if os.path.exists(vows_path):
        with open(vows_path, 'r', encoding='utf-8') as f:
            c = f.read()
        if "备婚精选 · 誓词与仪式好物" not in c:
            c = c.replace('<div class="related">', GOODS_VOWS + '\n  <div class="related">')
            with open(vows_path, 'w', encoding='utf-8') as f:
                f.write(c)
            print("Updated vows.html with JD goods cards")

    # 2. Update checklist.html
    checklist_path = os.path.join(PROJECT_ROOT, "checklist.html")
    if os.path.exists(checklist_path):
        with open(checklist_path, 'r', encoding='utf-8') as f:
            c = f.read()
        if "备婚刚需 · 筹备清单急救与接亲好物" not in c:
            c = c.replace('</main>', GOODS_CHECKLIST + '\n</main>') if '</main>' in c else c.replace('</body>', GOODS_CHECKLIST + '\n</body>')
            with open(checklist_path, 'w', encoding='utf-8') as f:
                f.write(c)
            print("Updated checklist.html with JD goods cards")

    # 3. Update qr-poster.html
    qr_path = os.path.join(PROJECT_ROOT, "qr-poster.html")
    if os.path.exists(qr_path):
        with open(qr_path, 'r', encoding='utf-8') as f:
            c = f.read()
        if "现场布置 · 签到台与水牌物料精选" not in c:
            c = c.replace('</main>', GOODS_QR + '\n</main>') if '</main>' in c else c.replace('</body>', GOODS_QR + '\n</body>')
            with open(qr_path, 'w', encoding='utf-8') as f:
                f.write(c)
            print("Updated qr-poster.html with JD goods cards")
